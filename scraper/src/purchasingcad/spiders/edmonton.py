"""Edmonton municipal tenders spider via SAP Ariba Discovery.

Edmonton exposes a public buyer profile page on SAP Ariba Discovery
(service.ariba.com/Discovery.aw) with server-rendered RJS listings.
ANID: AN01394774623 — confirmed 171 listings, public, no login required.

Same Ariba Discovery pattern as CalgarySpider — different ANID and
buyer_org/province remain AB. Pagination uses session-based tokens;
DOM click required (URL construction fails per Research Pitfall 3).
...
public-facing buyer directories designed to be found by suppliers.
"""
import logging
...
# Edmonton's public Ariba Discovery buyer profile page
_EDMONTON_PROFILE_URL = "https://service.ariba.com/Discovery.aw/ad/profile?key=AN01394774623"

# Safety bound to prevent infinite pagination on session-based Ariba pages
_MAX_PAGES = 20


class EdmontonSpider(Spider):
    """Scrapy Spider that ingests Edmonton municipal tenders from SAP Ariba Discovery.
    ...
    async def parse(self, response, **kwargs):
        """Parse Edmonton Ariba Discovery profile page for tender listings.
        ...
        try:
            ...
            if items_yielded == 0:
                logger.warning(
                    "Edmonton spider: No tender rows found at %s (page %d). "
            ...
            if page and page_num < _MAX_PAGES and items_yielded > 0:
                ...
                if next_btn:
                    logger.debug(
                        "Edmonton spider: Clicking next-page (page %d -> %d)",
                        page_num,
                        page_num + 1,
                    )
    ...
    def _extract_from_row(self, row, response) -> dict | None:
        ...
        for cell in cells:
            ...
            # Reference/external_id: numeric patterns
    ...
    def _record_to_item(self, record: dict) -> TenderItem:
        ...
        source_slug='edmonton', value_currency='CAD', title_fr=None
        (Ariba Discovery Edmonton portal is EN-only).
        """
        return TenderItem(
            source_slug="edmonton",
            ...
            description=None,
            description_fr=None,
            buyer_org=record.get("buyer_org", "City of Edmonton"),
            buyer_id=None,
            ...
            value_currency="CAD",
            source_url=record.get("url", _EDMONTON_PROFILE_URL),
            raw_ocds=record,
        )
    ...
    async def _errback(self, failure):
        ...
        if page:
            await page.close()
        logger.error(
            "Edmonton spider: Request failed for %s — %s",
            failure.request.url,
            failure.value,
        )
