"""Ottawa Municipal Tenders spider via MERX (merx.com/cityofottawa).
...
- Portal: https://www.merx.com/cityofottawa/solicitations/open-bids
...
- jurisdiction="muni" (NOT "prov") — Ottawa is a municipal government
"""
import logging
...
# Ottawa MERX open bids listing
_OTTAWA_START_URL = "https://www.merx.com/cityofottawa/solicitations/open-bids"

# Safety bound to prevent infinite pagination
_MAX_PAGE = 20


class OttawaSpider(Spider):
    """Scrapy Spider that ingests Ottawa municipal tenders from MERX (merx.com/cityofottawa).
    ...
    Yields TenderItems with jurisdiction="muni" and province="ON".
    """
    ...
    def parse(self, response):
        """Parse a MERX Ottawa tender listing page.
        ...
        if "captcha" in page_text_lower or response.status == 429:
            logger.warning(
                "Ottawa spider: Captcha or rate-limit detected at %s (status=%s) — "
                "skipping page.",
                response.url,
                response.status,
            )
        ...
        if items_yielded == 0:
            logger.warning(
                "Ottawa spider: No tender links found at %s — "
    ...
    def _record_to_item(self, record: dict) -> TenderItem:
        ...
        Fixed fields: province='ON', jurisdiction='muni',
        source_slug='ottawa', value_currency='CAD', title_fr=None
        (Ottawa MERX portal is English-only).
        """
        return TenderItem(
            source_slug="ottawa",
