"""Manitoba Government Tenders spider via MERX (merx.com/mbgov).
...
- Portal: https://www.merx.com/mbgov/solicitations/open-bids
...
# Manitoba MERX open bids listing
_MB_START_URL = "https://www.merx.com/mbgov/solicitations/open-bids"
...
class ManitobaSpider(Spider):
    """Scrapy Spider that ingests Manitoba tenders from MERX (merx.com/mbgov).
    ...
    def parse(self, response):
        """Parse a MERX Manitoba tender listing page.
        ...
        if "captcha" in page_text_lower or response.status == 429:
            logger.warning(
                "Manitoba spider: Captcha or rate-limit detected at %s (status=%s) — "
                "skipping page.",
                response.url,
                response.status,
            )
        ...
        if items_yielded == 0:
            logger.warning(
                "Manitoba spider: No tender links found at %s — "
    ...
    def _record_to_item(self, record: dict) -> TenderItem:
        ...
        Fixed fields: province='MB', jurisdiction='prov',
        source_slug='manitoba', value_currency='CAD', title_fr=None
        (Manitoba MERX portal is English-only).
        """
        return TenderItem(
            source_slug="manitoba",
